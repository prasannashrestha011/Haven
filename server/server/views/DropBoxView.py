...
def dropbox_oauth(request):
    client_id =os.getenv('DROPBOX_APP_KEY')  # Replace with your Dropbox app's client ID
...
def dropbox_oauth_callback(request):
    ...
    if response.status_code != 200:
        return Response({"error": "Failed to get token", "details": response.json()}, status=500)
