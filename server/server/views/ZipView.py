...
class ZipView(ViewSet):
    ...
    def init_repo(self,req:Request):
        repo_name=req.query_params.get("repo")
        if not repo_name:
            return Response({"error":"repo name not provided"},status=400)
        response=DropBoxService.Init_Repo_Dir(repo_name=repo_name)
    ...
    def commit_content_on_repo(self, req: Request):
        ...
        return Response({"message":response_data["message"],"zip_id":zip_id},status=response_data["status"])
